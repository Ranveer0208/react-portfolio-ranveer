...
const Experience = () => {
   ...
   if (!EXPERIENCES) {
      throw new Error("Experince must be defined")
   }

   return <>
      <div className="border-b border-neutral-900 pb-4">
         <motion.h1 whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: -100 }} transition={{ duration: 1.5 }} className="my-16 text-center text-4xl">Experience</motion.h1>
         <div>
            {
               EXPERIENCES.map((experience, index) => (
                  <div key={index} className="mb-8 flex flex-wrap lg:justify-center">
                     ...
                     <motion.div whileInView={{ opacity: 1, x: 0 }} initial={{ opacity: 0, x: 100 }} transition={{ duration: 1.5 }} className="w-full max-w-xl lg:w-3/4">
                        ...
                        <p className="mb-4 text-neutral-400">{experience.description.trim().split('•').map((item, index) => (
                           item ? <li key={index} className="text-start">{item}</li> : null
                        ))}</p>
